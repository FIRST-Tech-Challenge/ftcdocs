...
---------------------------------
There are many best practices that teams should keep in mind regarding :term:`servo <Servo>` usage.

Voltage Compatibility
^^^^^^^^^^^^^^^^^^^^^
Teams should be aware of the voltage requirements of their servos and ensure
they are compatible with the power source. The REV :term:`Control Hub` and the REV
:term:`Expansion Hub` both provide 5V to servos, while the goBILDA Servo Power
...
Control or Expansion hub. Teams should also be mindful of the 20A :term:`fuse <Fuse>` on the
which limits the full electrical system.

Power Source
^^^^^^^^^^^^
The power from each servo port is only for powering servos connected to that
port. The :term:`Competition Manual` prohibits teams from mixing power between devices.
